...
const { DateTime } = luxon;
    ...
    function updateCountdown() {
      ...
      // Fecha de Navidad en Bolivia
      let christmas = DateTime.local(now.year, 12, 25, 0, 0, 0, { zone: "America/La_Paz" });

      // Si ya pasó Navidad este año, calcula para el próximo año
      if (now > christmas) {
        christmas = christmas.plus({ years: 1 });
      }

      const diff = christmas.diff(now, ["days", "hours", "minutes", "seconds"]);

      endMessage.textContent = `Faltan ${Math.floor(diff.days)} días, ${diff.hours} horas, ${diff.minutes} minutos y ${Math.floor(diff.seconds)} segundos para Navidad.`;
    }
